...
curl -H "Authorization:Bearer ${ACCESS_TOKEN}" "${CONTROLLER_HOST}/controller/rest/applications/AppDynamics%20Analytics-2/metric-data?metric-path=Analytics%7CInstrumentable%20Tomcat%20Call%20Errors&time-range-type=BEFORE_NOW&duration-in-mins=${DURATION_MINS}"
